...
hold on
plot(tbot, loglikelihoodphi_t(pfxform(theta)),'*')
legend ('true params', 'guessed params')
xlabel('time(hours)')
ylabel('LL(mod;data)')
title('Probability at each time point')
set(gca,'FontSize',20,'LineWidth',1.5)
guessum = sum(loglikelihoodphi_t(pfxform(theta)))
truesum = sum(loglikelihoodphi_t(pfxform(pfset)))
%%
negLLphi = @(phat)-sum(loglikelihoodphi_t(phat));
objfun = @(phat)-sum(loglikelihoodphi_t(phat));
