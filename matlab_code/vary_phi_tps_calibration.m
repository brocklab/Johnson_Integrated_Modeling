...
% THIS SCRIPT VARIES THE NUMBER OF PHI TIME POINTS AND REPORTS RESULTING
% ACCURACY IN PARAMETERS AND OUPUTS
...
%% Generate dosed data and fit it using puntfit as you vary time points in phi(t)
...
timevec{1} = [0, 1344];
timevec{2}=[ 0, 30, 1344];
timevec{3} = [0, 30, 672, 1344];
timevec{4} = [0, 30, 336, 672, 1008];
timevec{5} = [0:24:1344];

tres = [672, 336, 168, 84, 42, 24, 8];
ntps = (tgen(end)./tres)+1;
...
%% Take stored data and 
for m = 1:length(ntps)
     %m is indicator for the time resolution
...
figure;
subplot(1,3,1)
plot(ntps, CCC_p, 'ro-', 'LineWidth',3)
xlabel('Number of time points')
ylabel('CCC_{parameters}')
xlim([min(ntps) max(ntps)])
title('Parameter concordance')
set(gca,'FontSize',20,'LineWidth',1.5)
subplot(1,3,2)
plot(ntps, CCC_N, 'bo-', 'LineWidth',3)
xlim([min(ntps) max(ntps)])
xlabel('Number of time points')
ylabel('CCC_{N}')
...
set(gca,'FontSize',20,'LineWidth',1.5)
subplot(1,3,3)
plot(ntps, CCC_phi, 'go-', 'LineWidth',3)
xlabel('Number of time points')
xlim([min(ntps) max(ntps)])
ylabel('CCC_{\phi}')
...
figure;
plot(ntps, mean_param_err,'ko-', 'LineWidth',3)
xlabel('Number of \phi time points')
ylabel('Average parameter error')
xlim([min(ntps) max(ntps)])
title('Average parameter error vs. number of \phi time points')
set(gca,'FontSize',20,'LineWidth',1.5)
...
figure;
for i = 1:nfit
    plot(ntps, CCC_pmat(i,:), '-o', 'LineWidth',2)
    hold on
end
set(gca,'FontSize',20,'LineWidth',1.5)
xlabel('Number of \phi time points')
...
title('CCC vs. number of \phi time points')
