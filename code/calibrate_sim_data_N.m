...
for i = 1:nsamps
    ...
    pfit2 = [S0, R0, pbestf(1), carcapfit, pbestf(2), pbestf(3), pbestf(4), dr];
        [Nb, ~,~]=fwd_Greene_model(pfit2, tbot, Ub, dt, tdrug);
        phi_model = Nb(:,2)./Nb(:,1);
...
legend ('in silico data', 'model fit', '95% CI on data', 'Location', 'NorthWest')
legend box off
title ('Example of N(t) fit to N(t)')
set(gca,'FontSize',20,'LineWidth',1.5)
subplot(1,2,2)
...
legend box off
title ('Example of \phi_{sens}(t) fit from N(t)')
set(gca,'FontSize',20,'LineWidth',1.5)
...
title(['N(t) fit from N(t), CCC= ', num2str(CCC_N)])
set(gca,'FontSize',20,'LineWidth',1.5)
subplot(1,3,3)
...
title(['\phi(t) fit from N(t), CCC= ', num2str(CCC_phi)])
set(gca,'FontSize',20,'LineWidth',1.5)
